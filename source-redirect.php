...
class SourceRedirect {
  ...
  function redirect() {
    ...
    $redirect_browser_chrome          =                                   get_option( 'redirect_browser_chrome',        '' );
    $redirect_browser_ie              =                                   get_option( 'redirect_browser_ie',            '' );
    $redirect_browser_firefox         =                                   get_option( 'redirect_browser_firefox',       '' );
    $redirect_browser_opera           =                                   get_option( 'redirect_browser_opera',         '' );
    $redirect_browser_safari          =                                   get_option( 'redirect_browser_safari',        '' );
    // Redirect Mobile Devices
    if( $redirect_mobile_all == 2 ) $redirect_mobile_url_all = false;
    if( $redirect_mobile_all == 1 or $redirect_mobile_all == 2 ) {
      SourceRedirect::mobile_device_detect( $redirect_mobile_url_all );
    }
    // Redirect Browsers
    if( $redirect_browser_all == 1 ) {
      if( strstr( strtolower($_SERVER['HTTP_USER_AGENT']), "chrome"  ) and $redirect_browser_chrome <> "" )  { wp_redirect( $redirect_browser_chrome ); exit; }
      if( strstr( strtolower($_SERVER['HTTP_USER_AGENT']), "msie"    ) and $redirect_browser_ie <> "" )      { wp_redirect( $redirect_browser_ie ); exit; }
      if( strstr( strtolower($_SERVER['HTTP_USER_AGENT']), "firefox" ) and $redirect_browser_firefox <> "" ) { wp_redirect( $redirect_browser_firefox ); exit; }
      if( strstr( strtolower($_SERVER['HTTP_USER_AGENT']), "opera"   ) and $redirect_browser_opera <> "" )   { wp_redirect( $redirect_browser_opera ); exit; }
      if( strstr( strtolower($_SERVER['HTTP_USER_AGENT']), "safari"  ) || strstr( strtolower($_SERVER['HTTP_USER_AGENT']), "konqueror" ) and $redirect_browser_safari <> ""  ) { wp_redirect( $redirect_browser_safari ); exit; }
    }
  } // ends function redirect()

  /*
   * Code credited by http://detectmobilebrowsers.mobi/
   */
  function mobile_device_detect($mobileredirect=false,$iphone=true,$ipad=true,$android=true,$nokia=true,$opera=true,$blackberry=true,$palm=true,$windows=true,$desktopredirect=false,$overwrite_opera_url=0) {
    ...
    switch ( true ) { // using a switch against the following statements which could return true is more efficient than the previous method of using if statements
      ...
      case (preg_match('/symbos/i',$user_agent));  // we find SymbOS in the user agent
        $mobile_browser = $nokia; // mobile browser is either true or false depending on the setting of nokia when calling the function
        $status = 'Nokia';
        if(substr($nokia,0,4)=='http'){ // does the value of nokia resemble a url
          $mobileredirect = $nokia; // set the mobile redirect url to the url value stored in the nokia value
        } // ends the if for android being a url
      break; // break out and skip the rest if we've had a match on nokia
    ...
    if($redirect = ($mobile_browser==true) ? $mobileredirect : $desktopredirect){
      wp_redirect( $redirect ); // redirect to the right url for this device
      exit;
    }else{
...
if ( !is_admin() ) {
	...
	add_action( 'init', array('SourceRedirect', 'redirect_this') );
	}
